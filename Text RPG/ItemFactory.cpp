...
std::vector<Item> ItemFactory::getScrolls()
{
	return {
		Item("Fireball Scroll",
			"불덩이를 발사 마법 스크롤",
			"거대한 불덩이가 적을 덮쳤습니다!",
			ItemType::SCROLL, 10, 40, "전체 직업"),

		Item("Healing Scroll",
			"즉시 체력 회복 마법 스크롤",
			"상처가 치유됩니다!",
			ItemType::SCROLL, 10, 50, "전체 직업"),
		...
		Item("Smoke Bomb Scroll",
			...
			ItemType::SCROLL, 10, 20, "전체 직업")

	};
}

std::vector<Item> g_ShopItemPool;

void RebuildShopItemPool() {
	g_ShopItemPool = ItemFactory::getAllItems();      
	auto scrolls = ItemFactory::getScrolls();         
	g_ShopItemPool.insert(g_ShopItemPool.end(),       
		scrolls.begin(), scrolls.end());
}


struct ShopPoolInitializer {
	ShopPoolInitializer() { RebuildShopItemPool(); }
} _shopPoolInitializer;